...
int getPosInfo::isInBackLane(Point p1, Point p2,float heading1)
{
  ...
  double lengthSum =pos_info1.trackS;
  ...
  while ((pos_info1.roadId != pos_info2.roadId) && (lengthSum < LengthSumMax))
  {
    ...
    // 需要判断1的后继与1的方向是否相同，若相同，继续使用后继，不相同则使用前继；
    //sunccessor类型为road
        ...
        ||((pos_info1.road->getPredecessorLink()!=NULL)&& GetRoadSuccessFlag==10))
    {
      ...
      //if have next sec
      {
        ...
        else if (nextroadI == 0)
        {
          ...
          //自车road的下一个road为road的getPredecessor，下一个road的第一个lane为自车所在road的最后一个lane的getPredecessor
          pos_info1old.road = pos_info1.road;
          pos_info1.road = pos_info1.road->getPredecessor();
          pos_info1.roadId = pos_info1.road->mId;
          ...
          //下一个road的第一个sec以及lane和sec的更新与下一个road方向是否与本road相同有关
          // 如果相同
    ...
    if ((pos_info1.road->getPredecessor(1))&&(pos_info1.road->getPredecessorLink()==NULL))
    {
      ...
      for (int RoadConnectI=0;RoadConnectI<(2*sizeof(RoadConnectId) / sizeof(RoadConnectId[0][0]));RoadConnectI++)
      {
        ...
        //待修改，根据road id 定位到对应的road 1067-1111
        OpenDrive::OdrManager myManager;
        OpenDrive::Position* pos = myManager.createPosition();
        myManager.activatePosition(pos);
        ...
        if (IncomRoadFlag==nextRoadDir)
        {
          ...
          else if (nextroadI == 0)
          {
            ...
            //自车road的下一个road为road的successor，下一个road的第一个lane为自车所在road的最后一个lane的successor
            pos_info1old.road = pos_info1.road;
            pos_info1.road = Connectroad;
            pos_info1.roadId = pos_info1.road->mId;
            std::cout << "2611pos_info1.roadId " << pos_info1.roadId << std::endl;
...
int getPosInfo::isInFrontLane(Point p1, Point p2,float heading1)
{
  ...
  if (pos_info1.roadId == pos_info2.roadId)
  {
    ...
    {
      bool endwhileFlag = 0;
      //如果点1的section在点2的后面，查找点1的下一个section，判断是否与点2相等；
      while ((pos_info1.roadId == pos_info2.roadId) && (pos_info1.sectionS < pos_info2.sectionS))
      {
        std::cout << "isInfrontsec " << std::endl;
        //getRight为获取下一个section的函数；
  ...
  double lengthSum =pos_info1.roadlength -pos_info1.trackS;
  std::cout << "lengthSum " <<lengthSum<< std::endl;
  bool endwhileFlag = 0;
  bool nextroadI = 0;
  posInfo pos_info1old = pos_info1;
...
bool getPosInfo::isFrontCar(Point p1, Point p2, float heading1, float heading2)
{
  bool isInRoad = getInertialPosInfo(p1);
  if (!isInRoad)
    //return -1;
  ...
  {
    int isInFrontOrBackLaneFlag = getPosInfo::isInFrontLane(p1, p2, heading1);
  ...
  {
    int isInFrontOrBackLaneFlag = getPosInfo::isInBackLane(p1, p2, heading1);
...
bool getPosInfo::isBackCar(Point p1, Point p2, float heading1, float heading2)
{
  bool isInRoad = getInertialPosInfo(p1);
  if (!isInRoad)
    return -1;
...
bool getPosInfo::isInSce2_46_7(Point p)
{
  ...
  // else
  {
    // 根据road id 定位到对应的road ,获取road初始位置的H值
    ...
    // 根据road id 定位到对应的road ,获取road的结束位置的H值
    posInfo pos_info1 = m_posInfo;
    while (pos_info1.section->getRight())
    {         
      OpenDrive::LaneSection *secInfo1 = pos_info1.section;
...
//TODO isSnow
bool getPosInfo::isInSec2_46_14(Point p)
{
  bool isInRoad = getInertialPosInfo(p);
